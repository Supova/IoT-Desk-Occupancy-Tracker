...
/* kicks off OTA job check then deletes itself */
static void ota_task(void *pvParameters) {
	(void) pvParameters;

	OTA_Status_t status = ota_start();
	if (status != OTA_SUCCESS) {
		LogError(("OTA start failed: %d", status));
	} else {
...
/* Routes incoming MQTT messages to OTA handler; logs unhandled messages */
void mqtt_receive_task(void *parameters) {
	...
	while (1) {
		...
		if (item.operation == MQTT_OPERATION_RECEIVE && item.topic_length > 0 && item.payload_length > 0) {
			bool handled = ota_handle_incoming_mqtt_message(
			    (char *)item.topic, item.topic_length,
			    (char *)item.payload, item.payload_length);
...
int main(void) {
	...
	BaseType_t status;
	status = xTaskCreate(ota_task, "OTA start", 512, NULL, 2, NULL);
