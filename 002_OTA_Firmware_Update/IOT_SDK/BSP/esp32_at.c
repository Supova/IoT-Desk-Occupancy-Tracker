...
esp32_status_t esp32_init(void) {
	...
	/* Construct the command */
	memset(at_cmd, '\0', MAX_AT_CMD_SIZE);
	sprintf((char*) at_cmd, "AT+CWMODE=%d%s", ESP32_STATION_MODE,
			AT_CMD_TERMINATOR);
...
esp32_status_t esp32_join_ap(uint8_t *ssid, uint8_t *password) {
	...
	memset(at_cmd, '\0', MAX_AT_CMD_SIZE);
	sprintf((char*) at_cmd, "AT+CWJAP=\"%s\",\"%s\"%s", ssid, password,
			AT_CMD_TERMINATOR);
 ...
 * otherwise.
 */
esp32_status_t esp32_establish_connection(
		const esp32_connection_info_t *connection_info) {
	esp32_status_t ret;
	...
	sprintf((char*) at_cmd, "AT+CIPSTART=\"TCP\",\"%s\",%lu%s",
			(char*) connection_info->ip_address, connection_info->port,
			AT_CMD_TERMINATOR);
...
esp32_status_t esp32_config_sntp(int utc_offset) {
	...
	sprintf((char*) at_cmd, "AT+CIPSNTPCFG=1,%d,\"%s\",\"%s\"%s", utc_offset,
...
esp32_status_t esp32_get_sntp_time(sntp_time_t *time_data) {
	...
	memset(at_cmd, 0, MAX_AT_CMD_SIZE);
	snprintf((char*) at_cmd, sizeof(at_cmd), "AT+CIPSNTPTIME?%s",
			AT_CMD_TERMINATOR);

	for (int retries = 0; retries < max_retries; retries++) {
		HAL_Delay(retry_delay_ms);
		if (run_at_cmd((uint8_t*) at_cmd, strlen((char*) at_cmd),
				(uint8_t*) AT_OK_STRING) != ESP32_OK) {
			continue;
		}
		...
		if (!p_sntp_time
				|| sscanf(p_sntp_time, "+CIPSNTPTIME:%3s %3s %d %d:%d:%d %d",
...
esp32_status_t esp32_mqtt_configure(const mqtt_connection_info_t *p_conn_info) {
	...
	if (p_conn_info == NULL || p_conn_info->p_host == NULL
			|| p_conn_info->p_port == 0) {
		return ESP32_ERROR;
	}
	...
	snprintf((char*) at_cmd, MAX_AT_CMD_SIZE,
			...
			p_conn_info->user_config.p_client_id ?
					p_conn_info->user_config.p_client_id : "",
			p_conn_info->user_config.p_username ?
					p_conn_info->user_config.p_username : "",
			p_conn_info->user_config.p_password ?
					p_conn_info->user_config.p_password : "",
	...
	snprintf((char*) at_cmd, MAX_AT_CMD_SIZE,
			...
			p_conn_info->conn_config.lwt_info.p_topic ?
					p_conn_info->conn_config.lwt_info.p_topic : "",
			(const char*) p_conn_info->conn_config.lwt_info.p_payload ?
					(const char*) p_conn_info->conn_config.lwt_info.p_payload :
 ...
 * @retval ESP32_OK on success, appropriate error code otherwise.
 */
esp32_status_t esp32_mqtt_connect_to_broker(
		const mqtt_connection_info_t *p_conn_info) {
	esp32_status_t ret;
	...
	memset(at_cmd, '\0', MAX_AT_CMD_SIZE);
	snprintf((char*) at_cmd, MAX_AT_CMD_SIZE, "AT+MQTTCONN=0,\"%s\",%u,%u%s",
			p_conn_info->p_host, p_conn_info->p_port, p_conn_info->reconnect,
			AT_CMD_TERMINATOR);
...
esp32_status_t esp32_mqtt_unsubscribe(const mqtt_subscribe_t *sub_args) {
	...
	memset(at_cmd, '\0', MAX_AT_CMD_SIZE);
	sprintf((char*) at_cmd, "AT+MQTTUNSUB=0,\"%s\"%s", sub_args->p_topic_filter,
			AT_CMD_TERMINATOR);
...
esp32_status_t esp32_mqtt_publish(const mqtt_publish_t *pub_args) {
	...
	sprintf((char*) at_cmd, "AT+MQTTPUBRAW=0,\"%s\",%u,%u,%u%s",
			pub_args->p_topic, pub_args->payload_length, pub_args->qos,
			pub_args->retain, AT_CMD_TERMINATOR);
	...
	ret = run_at_cmd((uint8_t*) pub_args->p_payload, pub_args->payload_length,
			(uint8_t*) AT_OK_STRING);
	...
	while (true) {
		if (esp32_io_recv_nb(&rx_char, 1) != 0) {
			...
			if (new_chunk == ESP32_TRUE) {
				if (length_value > 0) {
					p_receive_info->p_payload[p_receive_info->payload_length++] =
							rx_char;
					length_value--;
				} else {
		...
		if ((strstr((char*) rx_buffer, "+MQTTSUBRECV:0,") != NULL)
				&& (new_chunk == ESP32_FALSE)) {
			...
			while (esp32_io_recv_nb(&rx_char, 1) && rx_char != '"') {
				...
				p_receive_info->p_topic[p_receive_info->topic_length++] =
						rx_char;
			}
...
static esp32_status_t run_at_cmd(uint8_t *cmd, uint32_t length,
	...
	while ( true) {
		...
		/* Check if the message contains FAIL */
		if (strstr((char*) rx_buffer, AT_FAIL_STRING) != NULL) {
			return ESP32_ERROR;
		}
	}
